...
public class ProgressLoader extends View implements Animatable
{
    ...
    public ProgressLoader(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public ProgressLoader(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attributeSet) {
        PrConfiguration configuration = readFromAttributes(attributeSet);
    ...
    private PrConfiguration readFromAttributes(AttributeSet attributeSet) {
        PrConfiguration configuration = new PrConfiguration(getContext());

        TypedArray array = getContext().obtainStyledAttributes(attributeSet, R.styleable.ProgressLoader);

        for (int i = 0; i < array.length(); i++) {
            int type = array.getIndex(i);

            if (type == R.styleable.ProgressLoader_arc_style) {
                String value = array.getString(R.styleable.ProgressLoader_arc_style);

                configuration.setLoaderStyle(STYLE.values()[Integer.parseInt(value)]);
            }

            if (type == R.styleable.ProgressLoader_arc_colors) {
                int colors_resourse_id = array.getResourceId(R.styleable.ProgressLoader_arc_colors, 0);

                if (colors_resourse_id != 0)
                    configuration.setColors(getContext().getResources().getIntArray(colors_resourse_id));
            }

            if (type == R.styleable.ProgressLoader_arc_speed) {
                String value = array.getString(R.styleable.ProgressLoader_arc_speed);

                if(value != null)
                    configuration.setAnimationSpeedWithIndex(Integer.parseInt(value));
            }

            if (type == R.styleable.ProgressLoader_arc_margin) {
                Float value = array.getDimension(R.styleable.ProgressLoader_arc_margin, MARGIN_MEDIUM);

                configuration.setArcMargin(value.intValue());
            }

            if(type == R.styleable.ProgressLoader_arc_thickness)
            {
                Float value = array.getDimension(R.styleable.ProgressLoader_arc_thickness,getContext().getResources().getDimension(R.dimen.stroke_width));
    ...
    public void refreshArcLoaderDrawable(PrConfiguration configuration) {
        if(isRunning())
            stop();
    ...
    private static class ArcDrawable extends Drawable implements Animatable
    {
        ...
        PrConfiguration mConfiguration;
        Paint mPaint;
        ...
        public ArcDrawable(PrConfiguration configuration, View viewReference) {
